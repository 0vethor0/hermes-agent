...
- **Agent** (default ``ctrl+alt+a``) — toggles the OS-control voice agent
  (ElevenLabs). Independent from dictation hotkeys.
...
class HotkeyListener(QThread):
    ...
    Signals:
        ...
        hotkey_agent_toggled ():         Emitted on each agent hotkey press (toggle).
    ...
    def run(self) -> None:
        ...
        while self._running:
            try:
                ...
                agent_str = ConfigManager.get("HOTKEY_AGENT", "ctrl+alt+a")
                ...
                # -- Agent hotkey (toggle only) --
                if a_pressed and not agent_was_pressed:
                    self.hotkey_agent_toggled.emit()
                agent_was_pressed = a_pressed
