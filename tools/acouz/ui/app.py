...
class AcouZApp(QMainWindow):
    ...
    def _wire_backend(self) -> None:
        ...
        self.hotkey_listener.hotkey_agent_toggled.connect(self._toggle_os_agent)
        self.hotkey_listener.start()
        
        # ── Agente OS (Groq) ─────────────────────────────────────────
        from acouz.core.os_agent import GroqOSAgent
        from acouz.core.tts_notifier import TTSNotifier
        from acouz.core.wake_word import WakeWordListener

        self.tts = TTSNotifier()
        self.os_agent = GroqOSAgent(self)
        self.os_agent.set_speak_callback(self.tts.speak)
        self.wake_word = WakeWordListener(self)

        self.wake_word.detected.connect(self._on_wake_word)
        self.os_agent.action_executed.connect(self._on_os_action)
        self.os_agent.status_changed.connect(self.update_status)
        self.os_agent.session_ended.connect(self._on_agent_session_ended)

        # Notificaciones de voz en el dictado
        self.engine.status_changed.connect(self._speak_status)

        # Arranca la escucha de wake word si está habilitada
        if ConfigManager.get("WAKE_WORD_ENABLED", "true") == "true":
    ...
    def start_os_agent(self) -> None:
        """Inicia agente Groq (STT + LLM + acciones OS)."""
        if self.os_agent.is_running:
            return
        if self._is_dictating:
            self.update_status("Detén el dictado antes de activar el agente.")
            return
        self.tts.speak("Dime, estoy escuchando.")
        self.os_agent.start()
        self.update_status(tr("agent.started"))

    def stop_os_agent(self) -> None:
        if not self.os_agent.is_running:
            return
        self.os_agent.stop()
        self.update_status(tr("agent.stopped"))

    def _toggle_os_agent(self) -> None:
        if self.os_agent.is_running:
            self.stop_os_agent()
        else:
            self.start_os_agent()

    def _on_agent_session_ended(self) -> None:
        self.update_status(tr("engine.ready"))

    def _speak_status(self, status: str) -> None:
        """TTS solo para estados de dictado Groq (no mezclar con agente OS)."""
        if self.os_agent.is_running:
            return
        spoken = {
            "Grabando…":           "Grabando",
            "Listo para dictar":   "Texto listo",
            "Procesando…":         "Procesando",
        }
        msg = spoken.get(status)
        if msg and ConfigManager.get("ELEVENLABS_API_KEY"):
            self.tts.speak(msg)

    def _show_weather_ui(self, city: str) -> None:
        """Abre la página de clima cuando el agente lo solicita."""
        self.update_status(f"Mostrando clima: {city}")

    def _on_os_action(self, result: str) -> None:
        """Actualiza la UI cuando se ejecuta una acción del sistema."""
        self.update_status(f"Acción completada: {result[:80]}")
    ...
    def start_dictation(self) -> None:
        ...
        if self.os_agent.is_running:
            self.update_status("Detén el agente antes de dictar.")
            return
        if self._is_dictating:
            return
    ...
    def on_text_ready(self, text: str) -> None:
        """Dictado Groq terminado → pegar texto. Sin comandos OS aquí."""
        duration = (
    ...
    def refresh_home_shortcuts(self) -> None:
        ...
        hk_agent = ConfigManager.get("HOTKEY_AGENT", "ctrl+alt+a")
        self._add_shortcut_card(
            tr("home.shortcut.agent"), hk_agent, "agent", "zap", t
        )
    ...
    def _add_shortcut_card(
        ...
        action = (
            tr("home.shortcut.agent.toggle")
            if mode == "agent"
            else tr("home.shortcut.hold")
            if mode == "hold"
