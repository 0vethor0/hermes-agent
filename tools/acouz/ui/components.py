...
Reusable PySide6 UI components for the AcouZ application.

All components read the current theme via the module-level ``_t()`` accessor.
Styling is fully delegated to ``acouz.ui.styles``.
"""
...
def _t() -> Theme:
    """Return the currently active :class:`~acouz.ui.styles.Theme` object.
...
def set_theme(dark: bool) -> None:
    ...
    Called exclusively by :meth:`AcouZApp.apply_theme` before any
...
def _svg_data(key: str, color: str) -> bytes:
    ...
    Args:
        key:   Key into the :data:`~acouz.ui.styles.ICONS` dictionary.
...
def make_svg(key: str, size: int, color: str) -> QSvgWidget:
    ...
    Args:
        key:   Icon key defined in :data:`~acouz.ui.styles.ICONS`.
...
def reload_svg(widget: QSvgWidget, key: str, color: str) -> None:
    ...
    Args:
        ...
        key:    New icon key from :data:`~acouz.ui.styles.ICONS`.
...
def icon_label(key: str, text: str, t: Theme, size: int = 14) -> QWidget:
    ...
    Args:
        key:  Icon key from :data:`~acouz.ui.styles.ICONS`.
        text: Label text shown to the right of the icon.
        t:    Current :class:`~acouz.ui.styles.Theme` instance for styling.
...
class NavButton(QPushButton):
    ...
    Args:
        icon_key: Key in :data:`~acouz.ui.styles.ICONS` for the button icon.
...
class SettingRow(QWidget):
    ...
    def retheme(self, t: "Theme") -> None:  # noqa: F821
        ...
        Args:
            t: Active :class:`~acouz.ui.styles.Theme` instance.
        """
        self._lbl.setStyleSheet(S.setting_label_style(t))
        self._desc.setStyleSheet(S.setting_desc_style(t))

class CustomSettingsCard(QFrame):
...
class CustomSettingsCard(QFrame):
    ...
    def clear(self) -> None:
        """Remove all children from the card.
        """
...
def setting_row(label: str, desc: str, control: QWidget) -> SettingRow:
    """Convenience factory matching the old functional API.

    Args:
        label:   Primary label text.
        desc:    Secondary description text.
        control: Right-aligned control widget.

    Returns:
        A new :class:`SettingRow` instance.
    """
    return SettingRow(label, desc, control)
...
class StatCard(QFrame):
    ...
    Args:
        icon_key: Icon key from :data:`~acouz.ui.styles.ICONS`.
...
class HotkeyButton(QPushButton):
    ...
    key combination pressed by the user is saved via :class:`~acouz.utils.config.ConfigManager`.

    Args:
        config_key: :class:`~acouz.utils.config.ConfigManager` key to read/write.
    ...
    def _render(self) -> None:
        ...
        if self._listening:
            ...
            from acouz.utils.i18n import tr as _tr  # noqa: PLC0415
...
class TitleBar(QWidget):
    ...
    def _on_close(self) -> None:
        """Minimise to tray (mirrors :meth:`~acouz.ui.app.AcouZApp.closeEvent`)."""
        self._win.close()
