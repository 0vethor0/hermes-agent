...
from acouz.ui.components import (
    ...
    ToggleSwitch, HotkeyButton, SettingCard, StyledComboBox, CustomSettingsCard, btn_ghost,
)
...
class GeneralPage(BasePage):
    ...
    def _build(self) -> None:
        ...
        # OS agent shortcut -------------------------------------------
        self._lbl_agent = icon_label("zap", tr("general.hk.agent"), t, size=14)
        hkv.addWidget(self._lbl_agent)
        self._hk_agent_desc = QLabel(tr("general.hk.agent.desc"))
        self._hk_agent_desc.setStyleSheet(S.setting_desc_style(t))
        self._hk_agent_desc.setWordWrap(True)
        hkv.addWidget(self._hk_agent_desc)
        self._hk_btn_agent = HotkeyButton(
            config_key="HOTKEY_AGENT", default="ctrl+alt+a"
        )
        hkv.addWidget(self._hk_btn_agent)
        ...
        # ── Custom Settings ──────────────────────────────────────────
        self._sec_custom = section_title(tr("general.section.custom"))
        root.addWidget(self._sec_custom)
        self._custom_card = CustomSettingsCard()
        
        # Form to add new settings
        self._custom_form_card = SettingCard()
        self._key_input = QLineEdit()
        self._key_input.setPlaceholderText(tr("general.custom.key_placeholder"))
        self._val_input = QLineEdit()
        self._val_input.setPlaceholderText(tr("general.custom.value_placeholder"))
        self._add_btn = btn_ghost(tr("general.custom.add"))
        self._add_btn.clicked.connect(self._on_add_custom_setting)
        
        self._custom_form_card.add(tr("general.custom.key"), tr("general.custom.key_desc"), self._key_input)
        self._custom_form_card.add(tr("general.custom.val"), tr("general.custom.val_desc"), self._val_input)
        self._custom_form_card.add(tr("general.custom.action"), tr(""), self._add_btn)
        
        root.addWidget(self._custom_form_card)
    ...
    def retranslate(self) -> None:
        ...
        self._sec_custom._title_lbl.setText(tr("general.section.custom").upper())
        self._key_input.setPlaceholderText(tr("general.custom.key_placeholder"))
        self._val_input.setPlaceholderText(tr("general.custom.value_placeholder"))
        self._add_btn.setText(tr("general.custom.add"))
        self._refresh_custom_settings()
    ...
    def _on_add_custom_setting(self) -> None:
        """Handle adding a new custom configuration key-value pair.
        """
        key = self._key_input.text().strip()
        val = self._val_input.text().strip()
        if key:
            ConfigManager.set(key, val)
            self._key_input.clear()
            self._val_input.clear()
            self._refresh_custom_settings()

    def _on_remove_custom_setting(self, key: str) -> None:
        """Remove a custom configuration key.
        """
        # Since ConfigManager doesn't support deleting keys easily,
        # we can just set it to empty or something.
        # But a better way is to use a specialized method.
        # For now, let's just clear it.
        ConfigManager.set(key, "")
        self._refresh_custom_settings()

    def _refresh_custom_settings(self) -> None:
        """Refresh the custom settings list in the UI.
        """
        self._custom_card.clear()
        
        built_in_keys = {
            "GROQ_API_KEY", "WHISPER_MODEL", "LLM_MODEL", "MICROPHONE",
            "HOTKEY_DICTATE", "HOTKEY_CONTEXT", "HOTKEY_AGENT",
            "HOTKEY_DICTATE_MODE", "HOTKEY_CONTEXT_MODE", "CONFIRMATION_SOUND",
            "SHOW_OVERLAY", "UI_LANGUAGE", "DICTATION_LANGUAGE",
            "ELEVENLABS_API_KEY", "ELEVENLABS_AGENT_ID", "ELEVENLABS_VOICE_ID",
            "ELEVENLABS_TTS_MODEL", "WAKE_WORD_MODEL", "WAKE_WORD_ENABLED",
            "OS_CONTROL_ENABLED", "OS_CONTROL_FILES_DIR"
        }
        
        for key in ConfigManager.get_all_keys():
            if key and key not in built_in_keys and ConfigManager.get(key) != "":
                val = ConfigManager.get(key)
                del_btn = btn_ghost(tr("general.custom.remove"))
                # Use a default argument for key in the lambda to capture current key
                del_btn.clicked.connect(lambda checked, k=key: self._on_remove_custom_setting(k))
                self._custom_card.add(key, val, del_btn)
